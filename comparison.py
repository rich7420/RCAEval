...
📊 支持的數據集：
   - 基礎系列：online-boutique, sock-shop-1/2, train-ticket
   - RE系列：re1-ob/ss/tt, re2-ob/ss/tt, re3-ob/ss/tt
   - 多模態：multi-source (Metrics+Logs+Traces)

🔧 支持的 RCA 方法：
   - gnn_kan：基於圖神經網絡+KAN的深度學習方法
   - baro：基於貝葉斯在線變點檢測的統計方法
   - circa：基於因果推理的 RCA 方法
   - rcd：遞歸因果發現 RCA 方法

🔧 使用方法：
   # 比較多種 RCA 方法
   python comparison.py --methods gnn_kan baro circa rcd --datasets re2-tt --limit 10
   
   # 只測試特定方法
   python comparison.py --methods circa rcd --datasets online-boutique --limit 5
   
   # 數據量比較測試
   python comparison.py --methods gnn_kan circa --data-volume-test --volume-limits 2 5 10
"""
...
class GNNKANvsBAROComparator:
    ...
    def get_data_paths(self, dataset_name: str, limit: int = None) -> List[str]:
        """獲取數據集中的所有數據文件路徑 - 支持RE系列數據集結構和節點限制"""
        import glob
        ...
        # 檢查是否為合成數據集
        is_synthetic = dataset_info.get("synthetic", False)
        ...
        if data_paths:
            ...
            if limit and len(data_paths) > limit:
                ...
                for path in data_paths:
                    ...
                    if len(parts) >= 3:
                        service_fault = parts[-3]  # e.g., "ts-auth-service_delay"
                ...
                paths_per_type = max(1, limit // len(fault_types)) if fault_types else 0
                
                print(f"  📊 發現故障類型: {fault_types}")
                print(f"  📊 每種類型采樣 {paths_per_type} 個案例")
                ...
                print(f"  📊 最終選擇 {len(data_paths)} 個案例")
        else:
    ...
    def calculate_metrics(self, predicted_ranks: List[str], ground_truth: List[str], case_info: Dict = None) -> Dict[str, float]:
        ...
        self.metrics = [
            ...
            'recall@1', 'recall@2', 'recall@3', 'recall@5', 'recall@10',              # 🆕 添加 recall@2
            'f1@1', 'f1@2', 'f1@3', 'f1@5', 'f1@10',                                 # 🆕 添加 f1@2
            'avg@1', 'avg@3', 'avg@5', 'avg@10',  # 🆕 標準化後的 avg@k
            'mrr', 'ndcg@5', 'ndcg@10',
            'hit_rate@1', 'hit_rate@2', 'hit_rate@3', 'hit_rate@5',                   # 🆕 添加 hit_rate@2
            'average_precision'
        ]
        ...
        # 🆕 設置當前故障類型信息，供 fuzzy_match 使用
        if case_info and 'fault_type' in case_info:
            fuzzy_match.current_fault_type = case_info['fault_type']
        else:
            fuzzy_match.current_fault_type = None
        ...
        # 🔥 標準化 Avg@ 指標定義 - Mean Precision@k
        metrics['avg@1'] = metrics.get('precision@1', 0)  # avg@1 就是 precision@1
        metrics['avg@3'] = sum([metrics.get(f'precision@{k}', 0) for k in [1, 2, 3]]) / 3  # 🆕 標準定義
        # 對於 avg@5，我們需要計算所有可用的 precision@k (k=1,2,3,5)，然後除以實際計算的項數
        # 由於我們沒有計算 precision@4，所以 avg@5 = (p@1+p@2+p@3+p@5)/4
        metrics['avg@5'] = sum([metrics.get(f'precision@{k}', 0) for k in [1, 2, 3, 5]]) / 4   # 🆕 修正
        metrics['avg@10'] = sum([metrics.get(f'precision@{k}', 0) for k in [1, 2, 3, 5, 10]]) / 5  # 🆕 修正
    ...
    def run_data_volume_comparison(self, dataset_names: List[str] = None,
                                  volume_limits: List[int] = None) -> Dict[str, Any]:
        """運行不同數據量的比較測試"""
        if dataset_names is None:
            dataset_names = ["re2-tt"]  # 默認使用一個數據集
        
        if volume_limits is None:
            volume_limits = [2, 5, 10, 20]  # 默認測試不同數據量
        
        print(f"🔬 開始數據量比較測試")
        print(f"📊 測試數據集: {dataset_names}")
        print(f"📏 數據量限制: {volume_limits}")
        ...
        for limit in volume_limits:
            ...
            results = self.run_comparison(
                ...
                test_limit=limit,
                gnn_kan_configs={
                    'config_types': ['simplified'],
                    'feature_methods': ['kpca']
                }
            )
            
            all_results[f"limit_{limit}"] = results
            
            # 生成簡潔輸出
            simple_output = self.generate_simple_output()
            print(simple_output)
            ...
            for i, data_path in enumerate(tqdm(data_paths, desc=f"處理 {dataset_name}")):
                ...
                try:
                    ...
                    if baro_result["success"]:
                        ...
                        baro_metrics = self.calculate_metrics(baro_ranks, ground_truth, case_info)
                    ...
                    try:
                        ...
                        if gnn_kan_result["success"]:
                            ...
                            gnn_kan_metrics = self.calculate_metrics(gnn_kan_ranks, ground_truth, case_info)
    ...
    def calculate_dataset_summary(self, cases: List[Dict]) -> Dict[str, Any]:
        ...
        summary = {
            ...
            # 🆕 按故障類型分組的指標
            "metrics_by_fault_type": {"baro": {}, "gnn_kan": {}}
        }
        ...
        # 🆕 按故障類型分組收集指標
        baro_by_fault = defaultdict(list)
        gnn_kan_by_fault = defaultdict(list)
        
        for case in cases:
            fault_type = case["case_info"]["fault_type"]
            ...
            if case["methods"]["baro"]["success"]:
                ...
                # 🆕 按故障類型分組
                baro_by_fault[fault_type].append(case["methods"]["baro"]["metrics"])
            ...
            if case["methods"]["gnn_kan"]["success"]:
                ...
                # 🆕 按故障類型分組
                gnn_kan_by_fault[fault_type].append(case["methods"]["gnn_kan"]["metrics"])
        ...
        # 🆕 計算按故障類型分組的平均指標
        for method_name, fault_data in [("baro", baro_by_fault), ("gnn_kan", gnn_kan_by_fault)]:
            fault_averages = {}
            for fault_type, metrics_list in fault_data.items():
                if metrics_list:
                    ...
                    fault_averages[fault_type] = fault_avg
            summary["metrics_by_fault_type"][method_name] = fault_averages
    ...
    def generate_simple_output(self, dataset_name: str = None) -> str:
        """生成main.py風格的簡潔輸出"""
        if not self.results:
            return "❌ 沒有可用的比較結果"
        ...
        if dataset_name and dataset_name in self.results:
            ...
            dataset_results = list(self.results.values())
            dataset_names = list(self.results.keys())
        
        for i, (name, dataset_result) in enumerate(zip(dataset_names, dataset_results)):
            summary = dataset_result["summary"]
            
            if i > 0:
                output_lines.append("")  # 數據集間空行
            
            output_lines.append(f"📁 Dataset: {name}")
            output_lines.append(f"   Cases: {summary['total_cases']}")
            output_lines.append("")
            
            # BARO結果
            if summary['successful_cases']['baro'] > 0:
                baro_metrics = summary['average_metrics']['baro']
                baro_time = summary['average_execution_time']['baro']
                
                # Main.py風格的輸出 - BARO (按故障類型分組)
                output_lines.append("--- Evaluation results (BARO) ---")
                baro_fault_metrics = summary.get('metrics_by_fault_type', {}).get('baro', {})
                
                if baro_fault_metrics:
                    # 📊 詳細的分類指標顯示
                    
                    # Avg@1 按故障類型
                    output_lines.append("🎯 Avg@1 by fault type:")
                    for fault_type, metrics in baro_fault_metrics.items():
                        score = metrics.get('avg@1', 0.0)
                        fault_display = fault_type.upper()
                        if fault_type == 'mem':
                            fault_display = 'MEM'
                        elif fault_type == 'io':
                            fault_display = 'DISK'
                            ...
                            fault_display = 'DELAY'
                        output_lines.append(f"  Avg@1-{fault_display}:".ljust(14) + f" {score:.2f}")
                    
                    # Avg@3 按故障類型
                    output_lines.append("🎯 Avg@3 by fault type:")
                    for fault_type, metrics in baro_fault_metrics.items():
                        score = metrics.get('avg@3', 0.0)
                        fault_display = fault_type.upper()
                        if fault_type == 'mem':
                            fault_display = 'MEM'
                        elif fault_type == 'io':
                            ...
                            fault_display = 'DELAY'
                        output_lines.append(f"  Avg@3-{fault_display}:".ljust(14) + f" {score:.2f}")
                    
                    # Precision@1 按故障類型
                    output_lines.append("🎯 Precision@1 by fault type:")
                    for fault_type, metrics in baro_fault_metrics.items():
                        score = metrics.get('precision@1', 0.0)
                        fault_display = fault_type.upper()
                        if fault_type == 'mem':
                            fault_display = 'MEM'
                        elif fault_type == 'io':
                            ...
                            fault_display = 'DELAY'
                        output_lines.append(f"  P@1-{fault_display}:".ljust(14) + f" {score:.2f}")
                    
                    # Precision@3 按故障類型
                    output_lines.append("🎯 Precision@3 by fault type:")
                    for fault_type, metrics in baro_fault_metrics.items():
                        score = metrics.get('precision@3', 0.0)
                        fault_display = fault_type.upper()
                        if fault_type == 'mem':
                            fault_display = 'MEM'
                        elif fault_type == 'io':
                            ...
                            fault_display = 'DELAY'
                        output_lines.append(f"  P@3-{fault_display}:".ljust(14) + f" {score:.2f}")
                    
                    # 傳統的 Avg@5 (保持向後兼容)
                    output_lines.append("🎯 Avg@5 by fault type:")
                    for fault_type, metrics in baro_fault_metrics.items():
                        score = metrics.get('avg@5', 0.0)
                        fault_display = fault_type.upper()
                        if fault_type == 'mem':
                            fault_display = 'MEM'
                        elif fault_type == 'io':
                            ...
                            fault_display = 'DELAY'
                        output_lines.append(f"  Avg@5-{fault_display}:".ljust(14) + f" {score:.2f}")
                else:
                    # 回退到整體指標
                    output_lines.append(f"Avg@5-OVERALL: {baro_metrics.get('avg@5', 0.0):.2f}")
                
                output_lines.append("---")
                output_lines.append(f"Avg speed: {1/baro_time:.2f}")
                output_lines.append("")
            
            # GNN-KAN結果
            if summary['successful_cases']['gnn_kan'] > 0:
                gnn_kan_metrics = summary['average_metrics']['gnn_kan']
                gnn_kan_time = summary['average_execution_time']['gnn_kan']
                
                # Main.py風格的輸出 - GNN-KAN (按故障類型分組)
                output_lines.append("--- Evaluation results (GNN-KAN) ---")
                gnn_kan_fault_metrics = summary.get('metrics_by_fault_type', {}).get('gnn_kan', {})
                
                if gnn_kan_fault_metrics:
                    # 📊 詳細的分類指標顯示
                    
                    # Avg@1 按故障類型
                    output_lines.append("🎯 Avg@1 by fault type:")
                    for fault_type, metrics in gnn_kan_fault_metrics.items():
                        score = metrics.get('avg@1', 0.0)
                        fault_display = fault_type.upper()
                        if fault_type == 'mem':
                            fault_display = 'MEM'
                    ...
                    # 回退到整體指標
                    output_lines.append(f"Avg@5-OVERALL: {gnn_kan_metrics.get('avg@5', 0.0):.2f}")
                
                output_lines.append("---")
                output_lines.append(f"Avg speed: {1/gnn_kan_time:.2f}")
                output_lines.append("")
            
            # 簡潔比較
            if (summary['successful_cases']['baro'] > 0 and 
                summary['successful_cases']['gnn_kan'] > 0):
                
                baro_avg5 = summary['average_metrics']['baro'].get('avg@5', 0)
                gnn_kan_avg5 = summary['average_metrics']['gnn_kan'].get('avg@5', 0)
                
                output_lines.append("📊 Comparison:")
                output_lines.append(f"   BARO Avg@5:    {baro_avg5:.3f}")
                output_lines.append(f"   GNN-KAN Avg@5: {gnn_kan_avg5:.3f}")
                
                if gnn_kan_avg5 > baro_avg5:
                    diff = gnn_kan_avg5 - baro_avg5
                    improvement = (diff / baro_avg5) * 100 if baro_avg5 > 0 else 0
                    output_lines.append(f"   🏆 GNN-KAN wins by {diff:.3f} ({improvement:+.1f}%)")
                elif baro_avg5 > gnn_kan_avg5:
                    diff = baro_avg5 - gnn_kan_avg5  
                    output_lines.append(f"   🏆 BARO wins by {diff:.3f}")
                else:
                    output_lines.append(f"   🤝 Tie")
...
def main():
    ...
    parser.add_argument("--methods", nargs="+", 
                       choices=["gnn_kan", "baro", "circa", "rcd"],
                       default=["gnn_kan", "baro"],
                       help="要測試的 RCA 方法 (默認: gnn_kan baro)")
    ...
    parser.add_argument("--data-volume-test", action="store_true",
                       help="運行不同數據量的比較測試")
    parser.add_argument("--volume-limits", nargs="+", type=int, 
                       default=[2, 5, 10, 20],
                       help="數據量限制列表 (默認: 2 5 10 20)")
    parser.add_argument("--skip-detailed-report", action="store_true",
                       help="跳過詳細報告，只顯示簡潔結果")
    ...
    print("  🆕 Main.py風格輸出 - 簡潔的評估結果格式")
    print("  🆕 數據量比較測試 - 測試不同數據量的效果")
    print()
    if args.data_volume_test:
        print(f"🔬 數據量比較測試")
        print(f"📊 測試數據集: {args.datasets}")
        print(f"📏 數據量限制: {args.volume_limits}")
    else:
        print(f"📊 測試數據集: {args.datasets}")
    ...
    try:
        if args.data_volume_test:
            # 數據量比較測試
            print("🔬 運行數據量比較測試")
            results = comparator.run_data_volume_comparison(
                dataset_names=args.datasets,
                volume_limits=args.volume_limits
            )
        else:
            # 常規比較測試
            gnn_kan_configs = {
        ...
        # 數據量測試已經在run_data_volume_comparison中輸出結果了
        if not args.data_volume_test:
            # 生成簡潔輸出（main.py風格）
            simple_output = comparator.generate_simple_output()
            print("\n" + "="*50)
            print("📊 Simple Results (main.py style)")
            print("="*50)
            print(simple_output)
