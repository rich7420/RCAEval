"""
GAT (Graph Attention Network) RCA - Fair Baseline for GNN-KAN Comparison
...
def gat_rca(data, inject_time=None, dataset=None, with_bg=False, 
            config_type='simplified', feature_method='enhanced_ica', 
            use_optimized_input=True, **kwargs):
    """
    GAT RCA Analysis - Fair Baseline Method
    ...
    """
    print("Using GAT (Graph Attention Network) architecture for RCA analysis")
    print("Goal: Fair baseline comparison for GNN-KAN")
    ...
    # 3. Initialize GAT model (same architecture as GNN-KAN)
    print("Initializing GAT model (same layers and dimensions as GNN-KAN)...")
    ...
    # 4. Train GAT model (same training strategy as GNN-KAN)
    print("Starting GAT model training (same training strategy as GNN-KAN)...")
    from RCAEval.gnn_kan_module.gat_training import train_gat_model
    
    model, training_history = train_gat_model(
        model, 
        node_features, 
        edge_index, 
        config
    ...
    with torch.no_grad():
        try:
            ...
            print(f"GAT inference successful: input {node_features.shape} -> embeddings {embeddings.shape}")
            
            if adj_scores is not None:
                adj_matrix = torch.sigmoid(adj_scores)
                print(f"GAT graph structure learning successful: {adj_matrix.shape}, density: {adj_matrix.mean().item():.3f}")
            else:
            ...
            print(f"Warning: GAT inference failed: {inference_error}")
