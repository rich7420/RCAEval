"""
GATv2 (Dynamic Graph Attention Network) RCA - Improved GAT Baseline
...
def gatv2_rca(data, inject_time=None, dataset=None, with_bg=False, 
              config_type='simplified', feature_method='enhanced_ica', 
              use_optimized_input=True, **kwargs):
    """
    GATv2 RCA Analysis - Improved GAT Baseline Method
    ...
    - Uses dynamic attention mechanism (improved over standard GAT)
    ...
    """
    print("Using GATv2 (Dynamic Graph Attention Network) architecture for RCA analysis")
    print("Goal: Improved GAT baseline comparison for GNN-KAN")
    ...
    # 3. Initialize GATv2 model (same architecture as GNN-KAN)
    print("Initializing GATv2 model (same layers and dimensions as GNN-KAN)...")
    from RCAEval.gnn_kan_module.gatv2_model import GATv2Model
    
    model = GATv2Model(config, len(node_names))
    ...
    # 4. Train GATv2 model (same training strategy as GNN-KAN)
    print("Starting GATv2 model training (same training strategy as GNN-KAN)...")
    from RCAEval.gnn_kan_module.gatv2_training import train_gatv2_model
    
    model, training_history = train_gatv2_model(
        model, 
        node_features, 
        edge_index, 
        config
    ...
    with torch.no_grad():
        try:
            ...
            print(f"GATv2 inference successful: input {node_features.shape} -> embeddings {embeddings.shape}")
            
            if adj_scores is not None:
                adj_matrix = torch.sigmoid(adj_scores)
                print(f"GATv2 graph structure learning successful: {adj_matrix.shape}, density: {adj_matrix.mean().item():.3f}")
            else:
            ...
            print(f"Warning: GATv2 inference failed: {inference_error}")
